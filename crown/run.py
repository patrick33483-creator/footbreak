...
def _reverse_t5_drain_process(send, config) -> None:
    ...
    try:
        ...
        # The child fetches one immutable board, then serially claims and
        # merges jobs; it never bulk-claims a queue.  A reaped matcher stall
        # can therefore strand only its current job while fast earlier items
        # remain durable.
        send.send((
            "complete",
            drain_pending_jobs(config),
        ))
    except BaseException as exc:
        send.send(("error", type(exc).__name__))
    finally:
