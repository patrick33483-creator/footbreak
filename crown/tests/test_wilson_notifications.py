...
class CrownWilsonNotificationTest(unittest.TestCase):
    ...
    def test_low_odds_retry_and_multiple_markets_remain_distinct(self):
        ...
        message = self._native_message(low)
        self.assertIsNotNone(message)
    ...
    def test_native_message_never_reads_or_mentions_hkjc_counterpart(self):
        row = bet()
        message = self._native_message(row)
    ...
    def test_missing_stale_or_malformed_hkjc_artifacts_do_not_change_native_observation(self):
        ...
        row.pop("hkjc_match_id")
        message = self._native_message(row)
        self.assertIn("決定：不投注：賠率不足", message)
        self.assertIn("投注平台：皇冠", message)
        self.assertNotIn("馬會", message)
    ...
    def test_shared_budget_prioritizes_wilson_over_reciprocal_outbox(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(notify, "notification_lock", return_value=nullcontext(True)), \
                ...
                # Historical reciprocal rows are untouched and never enter
                # the native Crown Telegram path.
                self.assertEqual(notify.notify_new(ledger, config, max_attempts=1, max_seconds=5), 0)
                self.assertEqual(sender.call_count, 1)
