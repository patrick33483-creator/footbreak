...
class ManifestTest(unittest.TestCase):
 ...
 def test_multistage_level_two_definition_without_tier_path_is_valid(self):
  l=ledger();f=l["wilson_validation"]["conditions"][l["wilson_validation"]["condition_order"][1]];old=f["signature"];f["definition"]["miner_key"]=[x for x in f["definition"]["miner_key"] if not x.startswith("tier_path=")];f["definition"]["odds_trajectory"]="";new=definition_signature(f["definition"]);f["signature"]=new;v=f["evidence_versions"][0];v["condition_signature"]=new;rehash(v);f["active_evidence_hash"]=v["evidence_hash"];f["active_evidence"]={k:v.get(k) for k in ("version","cumulative_hits","cumulative_decided","wilson95_lower_raw","minimum_acceptable_odds_raw","minimum_acceptable_odds_display","activation_boundary_at","created_at","evidence_hash")};l["wilson_validation"]["condition_order"][1]=new;l["wilson_validation"]["conditions"][new]=l["wilson_validation"]["conditions"].pop(old);m=build_manifest(l,"footbreak");self.assertTrue(m["valid"],m);self.assertTrue(m["conditions"][1]["own_stage_matcher_can_structurally_admit"])
