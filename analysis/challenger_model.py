...
def build_feature_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in ordered:
        ...
        values: dict[str, float | None] = {
            ...
            "market_line": line,
            "market_odds": _finite(market.get("odds")),
            "stage_rank": STAGE_RANK.get(str(row.get("stage"))),
            "prior_stage_probability": prior,
            "stage_probability_delta": None if prior is None else probability - prior,
            "stage_changed": None if prior is None else float(abs(probability - prior) > 1e-12),
        ...
        previous[fixture_market] = probability
    return output
...
def fit_logistic(rows: list[dict[str, Any]]) -> tuple[TrainOnlyEncoder, list[float]]:
    """Fit deterministic full-batch L2 logistic regression with no dependencies."""
    encoder = TrainOnlyEncoder().fit(rows)
...
def evaluate_market(rows: list[dict[str, Any]], system: str, market: str, diagnostics: dict[str, int]) -> dict[str, Any]:
    ...
    encoder, coefficients = fit_logistic(train)
    champion_probability = [float(row["probability"]) for row in holdout]
    challenger_probability = predict(encoder, coefficients, holdout)
    ...
    return {
        **common, **periods,
        ...
        "challenger": {"metrics": challenger, "probability_artifact_written": False},
        "delta": gate["deltas"],
        "checks": gate["checks"],
